...
import 'package:bepop_ngu/data/models/classDetails.dart';
import 'package:bepop_ngu/data/models/installment.dart';
import 'package:bepop_ngu/data/models/paidFeeDetails.dart';
...
class ChildFeeDetails {
  ...
  double remainingInstallmentAmount() {
    ...
    for (var installment in paidInstallments()) {
      paidAmount = (installment.installmentAmount ?? 0.0) + paidAmount;
    }
  ...
  double maximumAdvanceInstallmentAmount() {
    // Calculate total amount of already paid installments
    double paidInstallmentAmount = 0.0;
    for (var installment in paidInstallments()) {
      paidInstallmentAmount += (installment.installmentAmount ?? 0.0);
    }
    ...
    // Calculate the amount that will be covered by future installments
    ...
    if (nextInstallment.id != null && !(nextInstallment.isPaid ?? false)) {
      ...
      if (!dueInsts.any((inst) => inst.id == nextInstallment.id)) {
        nextInstallmentAmount = nextInstallment.installmentAmount ?? 0.0;
      }
    }

    // Calculate outstanding amounts from due installments
    double outstandingAmount = 0.0;
    for (var installment in dueInsts) {
      outstandingAmount += (installment.installmentAmount ?? 0.0) +
    ...
    // Calculate the remaining amount that can be paid as advance
    double remainingAmount = totalFeeAmount -
        paidInstallmentAmount -
        nextInstallmentAmount -
        outstandingAmount;

    // Ensure the value is not negative
    return remainingAmount > 0.0 ? remainingAmount : 0.0;
  }
